...
- **BC:** `KafkaConsumer::consumeMatch()` удалён: диспетчеризация
  результата одного опроса по колбэкам дублировала потоковое чтение —
  те же сценарии (типобезопасная обработка результата, реакция на
