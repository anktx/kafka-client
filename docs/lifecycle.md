...
3. Ошибки нативного `close()` оборачиваются в `KafkaConsumerException` —
   как у всех остальных методов обёртки. Упавший `close()` консьюмера
   не закрывает (см. «Семантика упавшего `close()`»).
...
## Семантика упавшего `close()`

Флаг `$closed` выставляется **только после успешного** нативного
`close()`. Отсюда контракт упавшего закрытия:

- `close()`, завершившийся `KafkaConsumerException`, консьюмера
  **не закрывает** — идемпотентность защищает только успешное закрытие;
- повторный вызов `close()` снова проксируется в RdKafka: это
  единственный способ гарантировать попытку освобождения ресурсов,
  поэтому в `finally` его можно вызывать безусловно;
- guard остальных методов проверяет наш флаг, а не состояние нативного
  клиента, поэтому формально они остаются доступны. Состояние нативного
  клиента после упавшего `close()` ext-rdkafka не определяет: если
  ресурсы успели освободиться, вызовы упадут с ошибкой librdkafka и
  будут обёрнуты в `KafkaConsumerException` как любой другой сбой.

Практическое следствие: исключение из `close()` в `finally` нельзя ни
проглатывать молча, ни позволять ему затирать исходное исключение
teardown'а — логируйте его и/или ретраьте с backoff на уровне
приложения.

## Почему нет `__destruct`

Решение зафиксировано: клиенты библиотеки **не имеют деструкторов**,
закрытие — только явный вызов (`close()` у консьюмера, `flush()` у
продюсера). Причины:

1. **Наблюдаемость ошибок.** Момент вызова деструктора определяет
   GC/shutdown — вокруг него не построить `try/finally`, а исключение
   из `__destruct` либо роняет процесс, либо глотается. Это сломало бы
   контракт «ошибки закрытия типизированы и наблюдаемы».
2. **Неопределённый порядок разрушения.** При shutdown нативный handle
   может быть освобождён раньше обёртки — деструктор обратился бы к
   уже мёртвому ресурсу.
3. **Graceful shutdown требует управляемого момента.** Leave-group и
   финальное состояние смещений видны брокеру только при `close()` в
   правильной точке жизненного цикла приложения (signal handler,
   teardown DI-контейнера), а не «когда-нибудь» по решению GC.
4. **Утечки нет.** При разрушении обёртки ext-rdkafka освобождает
   собственные handle'ы сам — цена отказа от `__destruct` только в
   потере graceful leave-group, что и решает явный `close()`.

Альтернатива-«предохранитель» — `__destruct` с подавлением ошибок —
отклонена: молча проглоченный сбой закрытия хуже наблюдаемого
исключения из явного вызова.
