...
# Имя образа идёт последним аргументом docker run: переменные окружения
# (-e KAFKA_BROKERS в test-integration/test-all/test-file) пробрасываются
# между DOCKER_RUN и DOCKER_IMAGE — после имени образа docker трактует
# аргументы как команду контейнера.
DOCKER_RUN = docker run --rm -u "$(shell id -u):$(shell id -g)" -e HOME=/tmp -v "$(CURDIR):/app" -w /app
...
validate: ## Validate composer.json in strict mode
	$(DOCKER_RUN) $(DOCKER_IMAGE) composer validate --strict

test: ## Run PHPUnit unit tests
	@echo '$(BLUE)Running PHPUnit unit tests...$(NC)'
	$(DOCKER_RUN) $(DOCKER_IMAGE) vendor/bin/phpunit --testsuite=Unit --colors=never

test-integration: ## Run integration tests (requires Kafka at KAFKA_BROKERS, default localhost:9092)
	@echo '$(BLUE)Running integration tests...$(NC)'
	$(DOCKER_RUN) -e KAFKA_BROKERS="$(KAFKA_BROKERS)" $(DOCKER_IMAGE) vendor/bin/phpunit --testsuite=Integration --colors=never

test-all: ## Run all tests (unit + integration)
	@echo '$(BLUE)Running all tests...$(NC)'
	$(DOCKER_RUN) -e KAFKA_BROKERS="$(KAFKA_BROKERS)" $(DOCKER_IMAGE) vendor/bin/phpunit tests --colors=never

test-file: ## Run a single test file (usage: make test-file FILE=tests/Path/ToTest.php)
	@echo '$(BLUE)Running test file: $(FILE)$(NC)'
	$(DOCKER_RUN) -e KAFKA_BROKERS="$(KAFKA_BROKERS)" $(DOCKER_IMAGE) vendor/bin/phpunit $(FILE) --colors=never
...
test-coverage: ## Run PHPUnit tests with coverage (XML for filtered Infection runs)
	...
	$(DOCKER_RUN) $(DOCKER_IMAGE) vendor/bin/phpunit --testsuite=Unit --coverage-xml=.infection/coverage-xml --log-junit=.infection/coverage-xml/junit.xml

test-coverage-text: ## Run PHPUnit tests with coverage (text report)
	@echo '$(BLUE)Running PHPUnit tests with coverage (text)...$(NC)'
	$(DOCKER_RUN) $(DOCKER_IMAGE) vendor/bin/phpunit --testsuite=Unit --coverage-text --colors=never
...
coverage: ## Run PHPUnit tests with coverage and enforce the 100% line coverage gate
	...
	$(DOCKER_RUN) $(DOCKER_IMAGE) vendor/bin/phpunit --testsuite=Unit --coverage-text --coverage-php=.phpunit.cache/coverage.cov --colors=never
	$(DOCKER_RUN) $(DOCKER_IMAGE) php bin/coverage-check.php

infection: ## Run Infection mutation testing (same as composer infection)
	@echo '$(BLUE)Running Infection mutation testing...$(NC)'
	$(DOCKER_RUN) $(DOCKER_IMAGE) vendor/bin/infection --show-mutations
...
infection-test: ## Run Infection for a single test file (usage: make infection-test TEST=tests/Path/ToTest.php; requires make test-coverage)
	...
	$(DOCKER_RUN) $(DOCKER_IMAGE) vendor/bin/infection --coverage=.infection/coverage-xml --filter='$(TEST)' --show-mutations
...
infection-test-src: ## Run Infection for a source file (usage: make infection-test-src SRC=src/Path/ToFile.php; requires make test-coverage)
	...
	$(DOCKER_RUN) $(DOCKER_IMAGE) vendor/bin/infection --coverage=.infection/coverage-xml --filter='$(SRC)' --show-mutations
...
infection-test-src-skip: ## Run Infection for a source file skipping initial tests (usage: make infection-test-src-skip SRC=src/Path/ToFile.php; requires make test-coverage)
	...
	$(DOCKER_RUN) $(DOCKER_IMAGE) vendor/bin/infection --coverage=.infection/coverage-xml --filter='$(SRC)' --show-mutations --skip-initial-tests
...
infection-relaxed: ## Run Infection with relaxed MSI threshold (usage: make infection-relaxed SRC=src/Path/ToFile.php MSI=60; requires make test-coverage)
	...
	$(DOCKER_RUN) $(DOCKER_IMAGE) vendor/bin/infection --coverage=.infection/coverage-xml --filter='$(SRC)' --show-mutations --skip-initial-tests --min-msi=$(MSI) --min-covered-msi=$(MSI)

cs-fix: ## Run PHP CS Fixer
	@echo '$(BLUE)Running PHP CS Fixer...$(NC)'
	$(DOCKER_RUN) $(DOCKER_IMAGE) vendor/bin/php-cs-fixer fix -v --diff --show-progress=dots

cs-dry: ## Run PHP CS Fixer in dry-run mode
	@echo '$(BLUE)Running PHP CS Fixer (dry-run)...$(NC)'
	$(DOCKER_RUN) $(DOCKER_IMAGE) vendor/bin/php-cs-fixer fix --dry-run -v --diff --show-progress=dots

analyse: ## Run PHPStan static analysis (level 9)
	@echo '$(BLUE)Running PHPStan analysis (level 9)...$(NC)'
	$(DOCKER_RUN) $(DOCKER_IMAGE) vendor/bin/phpstan analyse --memory-limit=512M --no-progress

analyse-baseline: ## Generate PHPStan baseline
	@echo '$(BLUE)Generating PHPStan baseline...$(NC)'
	$(DOCKER_RUN) $(DOCKER_IMAGE) vendor/bin/phpstan analyse --memory-limit=512M --generate-baseline
